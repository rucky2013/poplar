...
public class DefaultControllerTranslator implements UrlToControllerTranslator {
    ...
    private HttpMethod getHttpMethod(HttpRequest request, String controllerName) {
        try {
            return HttpMethod.of(request);
        } catch (IllegalArgumentException e) {
            throw new MethodNotAllowedException(router.allowedMethodsFor(controllerName), request.getMethod().getName());
        }
    }
}
